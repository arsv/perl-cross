...
<ul>
	<li>If there's no <tt>-f</tt> among the options, just use
		<tt>-D<i>key=val</i></tt>.</li>
	<li>If there is <tt>-f</tt>, use <tt>-O -D<i>key=val</i></tt>.</li>
	<li>If you need to append to some existing value,
		use <tt>-A</tt>, it works in all cases.</li>
</ul>
<p>To change <tt>xconfig.sh</tt> instead of <tt>config.sh</tt>,
prepend <tt>--host</tt> to all options (<tt>--host-set <i>key=val</i></tt>).</p>

<p>Complete workflow, in case it is not clear where the value comes from:</p>
<ul>
	<li>configure starts and clears all the variables it knows from
	    the environment</li>
	<li>Command-line options are parsed left-to-right;
	<tt>-D</tt>, <tt>-U</tt>, <tt>--set</tt>, <tt>--define</tt> and other
	    similar options take effect immediately, setting their variables
	    without looking at any existing values.
	<li>After processing all the options, files supplied using -f are
	    loaded in the order they were specified in the command line,
	    overriding any values set so far.</li>
	<li>If -O was specified, -D/--set options are replayed, overwriting
	    whatever values the loaded files might have set.</li>
	<li>If mode is not set by this point, $target value is analyzed
	    and decision between native and cross is made.
	    For a cross-build, two copies of configure (miniperl and target)
	    are spawned and top-level configure exits.</li>
	<li>Hints are loaded. Hint values never override anything.</li>
	<li>Once all hints are processed, -A options are applied in the order
	    they were specified in the command line.</li>
	...
	<li>Variables that were set using -D/--set, or by any of the  hint
	    files, but which were not written yet, are appended to config.sh;
	    this step allows adding arbitrary variables to config.sh using
	    -D or equivalent options.</li>
